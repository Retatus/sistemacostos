...
<script setup>
    ...
    console.log(props);
    const form = useForm({
        nombre: props.tiposunat.nombre,
        estado_activo: props.tiposunat.estado_activo,
    });
    ...
    <AppLayout title="Dashboard">
        <template #header>
          <div class="flex justify-between">
              ...
              <Link :href="route('tipo_sunat')" class="btn btn-primary"> <i class="bi bi-plus"></i>
                  Lista TipoSunat
              </Link>                             
          </div>    
        </template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="flex justify-center overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <FormularioDinamico
                        ...
                        :fields="formFields"                            
                        :updating="true"
                        @submit="form.patch(route('tipo_sunat.update', tiposunat))" 
                    />
                </div>
            </div>
        </div>
