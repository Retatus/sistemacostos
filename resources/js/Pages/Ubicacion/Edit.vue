...
<script setup>
    ...
    const form = useForm({
        ...
        estado_activo: props.ubicacion.estado_activo,
    });
    ...
    <AppLayout title="Dashboard">
        <template #header>
          <div class="flex justify-between">
              ...
              <Link :href="route('ubicacion')" class="btn btn-primary"> <i class="bi bi-plus"></i>
                  Lista Ubicacion
              </Link>                             
          </div>    
        </template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="flex justify-center overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <FormularioDinamico
                        ...
                        :fields="formFields"                            
                        :updating="true"
                        @submit="form.patch(route('ubicacion.update', ubicacion))" 
                    />
                </div>
            </div>
        </div>
