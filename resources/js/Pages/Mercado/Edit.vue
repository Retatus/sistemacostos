...
<script setup>
    ...
    console.log(props);
    const form = useForm({
        nombre: props.mercado.nombre,
        estado_activo: props.mercado.estado_activo,
    });
    ...
    <AppLayout title="Dashboard">
        <template #header>
          <div class="flex justify-between">
              ...
              <Link :href="route('mercado')" class="btn btn-primary"> <i class="bi bi-plus"></i>
                  Lista Mercado
              </Link>                             
          </div>    
        </template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="flex justify-center overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <FormularioDinamico
                        ...
                        :fields="formFields"                            
                        :updating="true"
                        @submit="form.patch(route('mercado.update', mercado))" 
                    />
                </div>
            </div>
        </div>
