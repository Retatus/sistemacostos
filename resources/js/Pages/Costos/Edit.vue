...
<script setup>
    ...
    console.log(props);
    const form = useForm({
        nombre: props.costos.nombre,
        estado_activo: props.costos.estado_activo,
    });
    ...
    <AppLayout title="Dashboard">
        <template #header>
          <div class="flex justify-between">
              ...
              <Link :href="route('costo')" class="btn btn-primary"> <i class="bi bi-plus"></i>
                  Lista Costo
              </Link>                             
          </div>    
        </template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="flex justify-center overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <FormularioDinamico
                        ...
                        :fields="formFields"                            
                        :updating="true"
                        @submit="form.patch(route('costo.update', costo))" 
                    />
                </div>
            </div>
        </div>
