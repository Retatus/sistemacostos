...
use App\Factory\CotizacionDiaFactory;
use App\Factory\PasajeroFactory;

class CotizacionFactory
{
    public function __construct(
        private CotizacionDiaFactory $diaFactory,
        private PasajeroFactory $pasajeroFactory,
    ) {}

    public static function createEmpty(): CotizacionDTO
    {
    ...
    public static function fromArray(array $data): CotizacionDTO
    {
        return new CotizacionDTO(
            id: $data['id'] ?? null,
            ...
            //     : null,
            dias: array_map(
                fn($dia) => CotizacionDiaFactory::fromArray($dia),
                $data['dias'] ?? []
            ),
            pasajeros: array_map(
    ...
    public function fromModel(Cotizacion $model): CotizacionDTO
    {
        return new CotizacionDTO(
            id: $model->id,
            ...
            dias: $model->dias
                ->map(fn($dia) => $this->diaFactory->fromModel($dia))
                ->toArray(),

            pasajeros: $model->pasajeros
                ->map(fn($p) => $this->pasajeroFactory->fromModel($p))
                ->toArray(),
        );
    }
}
