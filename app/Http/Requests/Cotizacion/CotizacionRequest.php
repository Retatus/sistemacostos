...
class CotizacionRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'PasajeroServicio.*.dia' => 'required|integer|min:1',
            'PasajeroServicio.*.detalle.*.pasajero.nombre' => 'required|string|max:255',
            'PasajeroServicio.*.detalle.*.pasajero.apellido_paterno' => 'required|string|max:255',
            'PasajeroServicio.*.detalle.*.pasajero.apellido_materno' => 'required|string|max:255',
            'PasajeroServicio.*.detalle.*.pasajero.documento_numero' => 'required|string|max:20',
            'PasajeroServicio.*.detalle.*.pasajero.documento_file' => 'required|file|mimes:pdf',
            'PasajeroServicio.*.detalle.*.pasajero.documento_file_name' => 'required|string|max:255',
            'PasajeroServicio.*.detalle.*.pasajero.urlPrevisualizacion' => 'required|string|max:255',
            'PasajeroServicio.*.detalle.*.servicio_detalle.*.id' => 'required|integer|exists:servicio_detalles,id',
            'PasajeroServicio.*.detalle.*.servicio_detalle.*.nro_orden' => 'required|integer|min:1',
            'PasajeroServicio.*.detalle.*.servicio_detalle.*.servicio_id' => 'required|integer|exists:servicios,id',
            'PasajeroServicio.*.detalle.*.servicio_detalle.*.itinerario_destino_id' => 'required|integer|exists:itinerario_destinos,id',
            'PasajeroServicio.*.detalle.*.servicio_detalle.*.proveedor_categoria_id' => 'required|integer|exists:proveedor_categorias,id',
            'PasajeroServicio.*.detalle.*.servicio_detalle.*.observacion' => 'required|string|max:255',
            'PasajeroServicio.*.detalle.*.servicio_detalle.*.monto' => 'required|numeric|regex:/^\d+(\.\d{1,2})?$/',
            'PasajeroServicio.*.detalle.*.servicio_detalle.*.estado_activo' => 'required|integer|in:0,1',
        ];
    }
    public function messages(): array
    {
        return [
            ...
            'destino_turistico_detalle.*.id' => 'required|integer|exists:destinos,id',
            'destino_turistico_detalle.*.nombre' => 'required|string|max:255',
            'destino_turistico_detalle.*.nombre.string' => 'El nombre del destino turistico debe ser una cadena de texto.',
            'destino_turistico_detalle.*.nombre.max' => 'El nombre del destino turistico no debe superar los 255 caracteres.',
            'destino_turistico_detalle.*.nombre.required' => 'El nombre del destino turistico es obligatorio.',
